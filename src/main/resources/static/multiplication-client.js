function updateMultiplication() {
	//var basicUrl = $(location).attr('host');//http://localhost:8091
	var basicUrl = window.location.origin;//http://localhost:8091
    $.ajax({
        //url: "http://localhost:8091/multiplications/random"
    	url: basicUrl+"/multiplications/random"
    }).then(function(data) {
        // Cleans the form
...
function updateStats(alias){
	//var basicUrl = $(location).attr('host');
	var basicUrl = window.location.origin;
	$.ajax({
		url: basicUrl+"/results?alias=" + alias
	}).then(function(data) {
        $('#stats-body').empty();
...
$(document).ready(function(){
        ...
        $("#attempt-form").submit(function(event){
        	...
        	//Compose the data in the form that the API is expecting
